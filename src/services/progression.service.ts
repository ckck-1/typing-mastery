/**
 * Progression service — XP, levels, ranks, achievements.
 * Persists per-user via localStorage; same swap-in interface so it can move
 * to a real backend later without touching components.
 */
...
export const progressionService = {
  async get(): Promise<Progression> {
    return request("GET", "/progression", () => {
  ...
  async award(input: {
    ...
    return request("POST", "/progression/award", () => {
      const id = uid(); if (!id) throw new Error("Sign in required");
